...
public class ConfigureOllama {
	...
	public void configure() {
		...
		dialog.setTitle("Configure Ollama");
		...
		JLabel info = new JLabel("Specify host and port on which Ollama runs");
		c.gridx = c.gridy = 0;
		...
		dialog.getContentPane().add(info, c);
		...
		cancel.addActionListener(l -> dialog.dispose());
		...
		ok.addActionListener(l -> {
			...
			new SwingWorker<Boolean, Void>() {
				...
				protected Boolean doInBackground() {
					return test(dialog);   // now runs off the EDT
				}
				@Override
				protected void done() {
					ok.setEnabled(true);
		...
		System.out.println("dialog is done");
	}

	private void appendToOutput(JTextPane tp, String toAppend, Color c) {
		if(!toAppend.endsWith("\n"))
			toAppend += "\n";
		final String msg = toAppend;
		Runnable append = () -> {
	...
	public static void noOllama(Component parent, Ollama ollama) {
		try {
			SwingUtilities.invokeAndWait(()-> JOptionPane.showMessageDialog(parent,
	...
	public boolean modelDoesntExist(Component parent) {
		AtomicInteger res = new AtomicInteger();

		try {
			SwingUtilities.invokeAndWait(() -> res.set(JOptionPane.showOptionDialog(
					parent,
	...
	public static void main(String[] args) {
		...
		String model = "nlScript-i2k2026";
		String pathToGGUF = "D:\\nls\\nlScript-i2k2025\\src\\main\\resources\\model-q4_k_m.gguf";
		new ConfigureOllama(new Ollama(model, pathToGGUF)).configure();
	}
}
